...
export class ValuationsService {
  ...
  async create(body: { projectId?: string; applicantNic?: string; data?: string }, file?: Express.Multer.File) {
    ...
    const result = await this.db.transaction(async (client) => {
      // Serialize numbering only for this project. Other projects remain fully concurrent.
  ...
  async listByProject(projectId: string) {
    const r = await this.db.query(
      `SELECT v.id, v.valuation_id, v.project_id, v.status, v.created_at,
              v.technical_officer_id, to_char(v.assigned_date, 'YYYY-MM-DD') AS assigned_date, v.assigned_time,
              v.details->>'bankName' AS bank_name,
              v.details->>'bankBranchName' AS branch_name,
              u.first_name, u.last_name
         FROM valuations v
         LEFT JOIN users u ON u.user_id = v.technical_officer_id
    ...
    return r.rows.map((row) => ({
      ...
      technicalOfficerName: `${row.first_name ?? ''} ${row.last_name ?? ''}`.trim(),
      inspectionDate: (row.assigned_date as string) || '',
      inspectionTime: (row.assigned_time as string) || '',
  ...
  // Every valuation in the system, newest first. Backs the coordinator's
  // Valuations page — the mentor asked for valuations to be a first-class
  // sidebar destination, not something only reachable through a project.
  async listAll() {
    const r = await this.db.query(
      `SELECT v.id, v.valuation_id, v.project_id, v.status, v.created_at,
              v.technical_officer_id, to_char(v.assigned_date, 'YYYY-MM-DD') AS assigned_date, v.assigned_time,
         ...
         LEFT JOIN users a ON a.user_id = p.applicant_nic
         LEFT JOIN users t ON t.user_id = v.technical_officer_id
    ...
    return r.rows.map((row) => ({
      ...
      createdAt: row.created_at as string,
      technicalOfficerId: (row.technical_officer_id as string) || '',
      technicalOfficerName: `${row.to_first_name ?? ''} ${row.to_last_name ?? ''}`.trim(),
      inspectionDate: (row.assigned_date as string) || '',
      inspectionTime: (row.assigned_time as string) || '',
      bankName: (row.bank_name as string) || '',
      branchName: (row.branch_name as string) || '',
