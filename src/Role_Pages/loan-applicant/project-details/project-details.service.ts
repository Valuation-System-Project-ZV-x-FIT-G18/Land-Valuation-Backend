...
export class ProjectDetailsService implements OnModuleInit {
  ...
  async attachment(draftId: number, docType: string) {
    ...
    const r = await this.db.query(
      `SELECT file_name, file_path, file_mime, file_data, object_key FROM applicant_project_detail_files
    ...
    return { fileName: f.file_name as string, filePath: f.file_path as string, mime: f.file_mime as string, data: objectData ?? f.file_data as Buffer | null }
  }
